...
| `K` | Turn album artwork on or off. The cover is drawn inline, right of the header, with the playback status above it; the visualizer narrows to share the row. Remembered in `cover_art`, sized by `cover_art_size`. Full layout tier and Spotify provider only |
