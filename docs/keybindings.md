...
| `K` | Turn album artwork on or off. The cover is drawn inline, right of the header, with the playback status under it; the visualizer narrows to share the row. Remembered in `cover_art`, sized by `cover_art_size`. Full layout tier only, and needs a provider that supplies artwork (Spotify, Navidrome, or a local file with embedded art) |
